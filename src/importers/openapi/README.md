...
Converts Swagger 2.0 and OpenAPI 3.0/3.1/3.2 documents (JSON or YAML) into the
normalized API model, then the deterministic catalog compiler produces an
inert catalog v2 artifact plus a structured import report.
...
The same importer compiles artifacts produced by the AI analysis skill
(`.opencode/skills/mcp-portico-analyze`). Pass `--ai` on the CLI or
